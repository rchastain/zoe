...
MoveScore alphabeta(Game game, int alpha, int beta, int depth) {
    ...
    for(move = 0; move < nmoves; move++) {
		...
		printf("%i %i %.2f %i ", depth, new.score, (float) ((nodes * CLOCKS_PER_SEC) / (clock() - start), nodes));	
    ...
    if(depth == SEARCHDEPTH) {
		...
		printf("%i %i %.2f %i ", depth, best.score, (float) ((nodes * CLOCKS_PER_SEC) / (clock() - start), nodes));	
