...
void edit_mode(Game *g)
{
    ...
    {
        if (g->board.b[BLACK][ROOK] & (1ull << 7)) g->can_castle[BLACK][KINGSIDE] = 1;
        if (g->board.b[BLACK][ROOK] & (1ull))      g->can_castle[BLACK][QUEENSIDE] = 1;
    }

    if (g->board.b[WHITE][KING] & (1ull << 60))
    {
        if (g->board.b[WHITE][ROOK] & (1ull << 63)) g->can_castle[WHITE][KINGSIDE] = 1;
        if (g->board.b[WHITE][ROOK] & (1ull << 56)) g->can_castle[WHITE][QUEENSIDE] = 1;
    }
}
